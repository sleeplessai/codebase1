...
template <typename Func> __global__ void ParallelFor(int n, Func func) {
...
int main(int argc, char const *argv[]) {
  ...
  constexpr auto genf = [] { return std::rand() * (1.f / RAND_MAX); };
  ...
  ParallelFor<<<20, 128>>>(
      n, [pi, x_dev = x_dev.data(), y_dev = y_dev.data()] __device__(int i) {
