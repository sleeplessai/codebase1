...
auto fetch_point_light_data(int index) {
    kit::CsvDatabase db("assets/common/point_light_attenuation.csv", "point_light_attenuation");
    db.open();
    db.buffer();
    db.show_meta();
...
#include "../assets/ch2/cube_norm_tex.inc"
    };
    std::vector<glm::vec3> cube_pos = {
#include "../assets/ch2/cube_pos.inc"
    };
    ...
    Material ml {
        .diffuse = kit::make_texture("assets/ch2/container2.png"),
        .specular = kit::make_texture("assets/ch2/container2_specular.png"),
        .shininess = 128.0f
    };
    ...
    Shader cube_shader("shaders/ch2/1-cube.vs", "shaders/ch2/1-cube.fs");

#if defined (__Paralleling_light_rendering)
    Shader paralleling_shader("shaders/ch2/4-tex.vert", "shaders/ch2/5-caster_paralleling.frag");
    paralleling_shader.use();
    paralleling_shader.setInt("material.diffuse", 0);
    paralleling_shader.setInt("material.specular", 1);
#elif defined (__Point_light_rendering)
    Shader point_shader("shaders/ch2/4-tex.vert", "shaders/ch2/5-caster_point.frag");
    point_shader.use();
    point_shader.setInt("material.diffuse", 0);
    point_shader.setInt("material.specular", 1);
#elif defined (__Spot_light_rendering)
    Shader spot_shader("shaders/ch2/4-tex.vert", "shaders/ch2/5-caster_spot.frag");
    spot_shader.use();
